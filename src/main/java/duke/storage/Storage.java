...
{
    public static final java.nio.file.Path DATA_STORAGE_PATH = java.nio.file.Paths.get(System.getProperty("user.dir"),
            "src", "main", "java", "duke", "data","data.txt");
     ...
     * @throws IOException if the file cannot be manipulated.
     */
    public static void appendNewTask(Task newTask) throws IOException {
        FileWriter fw = new FileWriter(DATA_STORAGE_PATH.toString(), true);
        String description = newTask.toString();
    ...
    public static void loadTasks() throws FileNotFoundException {
        ...
        while (s.hasNext()) {
            ...
            if (data.contains("[T]")) {
                ...
                Task t = new Event (event_description, time);
                TaskList.taskList.add(t);
                if (data.contains("[X]")) {
                    t.markAsDone();
                }
