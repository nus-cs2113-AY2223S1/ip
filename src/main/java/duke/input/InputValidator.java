...
public class InputValidator {
     ...
     */
    public static boolean isTerminatingInput(String input) {
        return input.equalsIgnoreCase(Duke.COMMAND_TERMINATE);
    }
     ...
     * @param input input string to check
     * @return boolean, true or false
     * @throws ListTooManyCommandsException If the {@link Duke#COMMAND_LIST} is not by itself
     */
    public static boolean isListInput(String input) throws ListTooManyCommandsException {
        if (input.equalsIgnoreCase(Duke.COMMAND_LIST)) {
            return true;
        } else if (stringContains(input, Duke.COMMAND_LIST)) {
            throw new ListTooManyCommandsException();
        } else {
            return false;
        }
    }
    ...
    private static boolean validateLength(String input, String command, String subCommand) throws
        ...
        int length = (input).split(subCommand).length;
        if (length == 2) {
     ...
     * @return boolean, true or false
     */
    public static boolean isMarkInput(String input) {
        return stringContains(input, Duke.COMMAND_MARK);
    }
     ...
     * @return boolean, true or false
     */
    public static boolean isUnmarkInput(String input) {
        return stringContains(input, Duke.COMMAND_UNMARK);
    }
